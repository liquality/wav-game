...
async function setup() {
  ...
  wavNFT = await WavNFT.deploy("https://remix.ethereum.org/");
...
describe("WavGame Contract", async function () {
  ...
  describe("Collect: Enter game island 1 ", async function () {
    it("Should revert if insufficient value sent", async () => {
      ...
      await expect(
        ...
        "InsufficientPayment(4000000000000000000, 1000000000000000000)"
      );
    });
    it("Should not mint NFT with zero amount", async () => {
      ...
      await expect(
        wavGame.connect(accounts[9]).collect(artist3, player1, [[12, 0]], {
          value: feePerMint,
        })
  ...
  describe("LevelUp Game", async () => {
    ...
    it("Should levelup successfully, if required burn condition is met", async () => {
      ...
      expect(await wavNFT.isApprovedForAll(player1, wavGame.address)).to.equal(
        true
      );
  ...
  describe("Prized collectors", async () => {
    it("Should only count unique collectors per island per artist/game", async () => {
      ...
      const prevCollectorCount = (
        await wavGame.fetchPrizedCollectors(artist1, ENTRY_LEVEL)
      ).length;

      await wavGame.connect(accounts[9]).collect(
        artist1,
        player1,
        [
          [1, 3],
      ...
      expect(
        (await wavGame.fetchPrizedCollectors(artist1, nextIsland)).length -
          prevCollectorCount
    ...
    it("Should not increase prizedCollectors count after prizeCutoff", async () => {
      ...
      await wavGame.connect(accounts[9]).collect(artist3, player1, [[11, 2]], {
        value: feePerMint.mul(ethers.BigNumber.from(2)),
      });
      await wavGame.connect(accounts[8]).collect(artist3, player2, [[11, 2]], {
        value: feePerMint.mul(ethers.BigNumber.from(2)),
      });
      await wavGame.connect(accounts[7]).collect(artist3, player3, [[11, 2]], {
        value: feePerMint.mul(ethers.BigNumber.from(2)),
      });
  ...
  describe("Game administration", async () => {
    it("should only allow admin set details", async () => {
      ...
      await expect(
        wavGame
          .connect(accounts[5])
          .setFeePerMint(ethers.utils.parseEther("2.0")) // owner is accounts[0]
      ...
      await expect(
        wavGame
          .connect(accounts[5])
          .wavMint(player1, artist1, ENTRY_LEVEL, 1, 2)
      ).to.revertedWith("Ownable: caller is not the owner");
      await expect(
    ...
    it("Should send complete payment to payment address when forwardValue is called", async () => {
      ...
      expect(
        (await ethers.provider.getBalance(artist1)).sub(prevBal1)
      ).to.equal(
        pendingPayment1 // Sum total in contract for payments on artist 1 NFTs
      );
      expect(
        (await ethers.provider.getBalance(artist2)).sub(prevBal2)
      ).to.equal(
        pendingPayment2 // Sum total in contract for payments on artist 2 NFTs
      );
      expect(
        (await ethers.provider.getBalance(artist3)).sub(prevBal3)
      ).to.equal(
    ...
    it("should update game island info correctly per artist ", async () => {
      ...
      expect(
        (await wavGame.getIsland(artist1, ENTRY_LEVEL))[0].requiredBurn
